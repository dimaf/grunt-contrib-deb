...
Deb.prototype.pack = function (definition, files, callback) {
  ...
  async.series([
    ...
    function buildDebBinFile(done) {
      fs.writeFile(getPath('debian-binary', definition), '2.0\n', done);
    },
    function buildPackage(done) {
      ...
      var writer = new ar.ArWriter(pkgPath, {variant: 'gnu',uid: '0', gid: '0'});
      writer.writeEntries([
        getPath('debian-binary', definition),
        getPath('control.tar.gz', definition),
        getPath('data.tar.gz', definition)
      ], function (err) {
        if (err)
        ...
        // remove temp files
        async.parallel([
          fs.unlink.bind(fs, getPath('control.tar.gz', definition)),
          fs.unlink.bind(fs, getPath('data.tar.gz', definition)),
          fs.unlink.bind(fs, getPath('debian-binary', definition))
        ], done);
      });
    }
  ], callback);
...
function buildControlFile(definition, callback) {
  ...
  var dependencies = definition.info.dependencies ?
      definition.info.dependencies.join(',') : '';
  ...
  if (definition.controlFile){
    Object.assign(self.controlFile,definition.controlFile);
  }
  ...
  async.parallel([
    ...
    var file = fs.createWriteStream(path.resolve(getPath('control.tar.gz', definition)));
...
function packFiles(files, definition, callback) {
  ...
  async.eachSeries(files, function (crtFile, done) {
    ...
    if (err) {
      ...
      var file = fs.createWriteStream(path.resolve(getPath('data.tar.gz', definition)));
      file.on('finish', callback);
...
function getPath(part, definition) {
    return path.join('.', definition.tmpPath || '', part);
}
